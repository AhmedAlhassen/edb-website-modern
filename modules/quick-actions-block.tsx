...
export function QuickActionsBlock({ block, locale }: { block: QuickActionsBlockType; locale: Locale }) {
  ...
  return (
    <>
      <section className="border-b border-border bg-surface dark:bg-[#0F172A] lg:hidden">
        <div className="grid grid-cols-2">
          {meta.map(({ Icon, title, href }, index) => (
            <Link key={title} href={href as never} className="group">
              <article className={`p-6 transition-colors hover:bg-surface-muted/60 active:bg-surface-muted ${index % 2 === 0 ? 'border-e border-border' : ''} ${index < 2 ? 'border-b border-border' : ''}`}>
      ...
      <section className="hidden border-b border-border bg-surface px-0 dark:bg-[#0F172A] lg:block">
        <div className="mx-auto grid w-full max-w-[1360px] grid-cols-4">
          {meta.map(({ Icon, title, href }, index) => (
            <Link key={title} href={href as never} className="group">
              <article className={`min-h-[270px] border-r border-border p-10 transition-colors hover:bg-slate-50 dark:hover:bg-[#1E293B] ${index === meta.length - 1 ? 'border-r-0' : ''}`}>
