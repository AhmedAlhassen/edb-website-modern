...
export function AnnouncementRailBlock({ block, locale }: { block: AnnouncementRailType; locale: Locale }) {
  ...
  return (
    <>
      <section className="border-b border-border bg-[#fcfcfd] px-4 py-12 dark:bg-[#0F172A] lg:hidden">
        ...
        <div className="flex flex-col gap-4">
          {block.items.map((item, index) => (
            <Card key={item.id} className={`relative rounded-xl border bg-surface p-5 ${index === 2 ? 'border-2 border-primary dark:border-primary' : 'border-border dark:bg-[#1E293B]'}`}>
              {index === 2 && (
              ...
              <Link
                ...
                className={`focus-ring typo-caption inline-flex h-11 w-full items-center justify-center rounded uppercase ${
                  index === 2
                    ...
                    : 'border-2 border-border text-slate-900 transition-colors hover:bg-surface-muted/70 active:bg-surface-muted dark:border-[#334155] dark:text-[#F8FAFC]'
                }`}
              >
                {labels.ctas[index]}
              </Link>
      ...
      <section className="hidden border-b border-border bg-[#fcfcfd] px-8 py-24 dark:bg-[#0F172A] lg:block">
        <div className="mx-auto w-full max-w-[1360px]">
          ...
          {block.items.map((item, index) => (
            <Card
              key={item.id}
              className={`relative flex h-full flex-col rounded-xl border bg-surface p-8 ${
                index === 2 ? 'border-primary/20 shadow-2xl shadow-primary/5 ring-1 ring-primary/5 dark:border-primary/30 dark:shadow-primary/10 dark:ring-primary/20' : 'border-border hover:shadow-xl dark:bg-[#1E293B] dark:hover:border-primary/50'
              }`}
            >
              {index === 2 && (
                <div className="absolute right-0 top-0">
              ...
              <Link
                href={item.href as never}
                className={`focus-ring typo-body-md inline-flex min-h-12 w-full items-center justify-center rounded-lg border-2 px-4 font-semibold ${
                  index === 2 ? 'border-primary bg-primary text-white hover:bg-blue-700' : 'border-slate-100 text-slate-900 hover:border-primary hover:text-primary dark:border-[#334155] dark:text-[#F8FAFC]'
                }`}
              >
                {labels.ctas[index]}
              </Link>
